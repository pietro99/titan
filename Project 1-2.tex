...
\begin{document}
	...
	There are 62 moons orbiting around Saturn, the twentieth-most distant of these is called Titan. This moon is larger than Mercury, which makes it the second-largest moon in the Solar System, only topped by Jupiter?s Ganymede. It was discovered in 1655 by the Dutch astronomer Christiaan Huygens, after whom the space Cassini-Huygens space mission was named. Titan is the only moon in the Solar System with a dense atmosphere, which was previously mistaken for a bigger size until the previously mentioned space mission. The biggest shock this mission revealed, however, was the discovery of liquid hydrocarbon lakes in its polar regions. Its atmosphere consists mostly of nitrogen. This creates ethane and methane clouds, which results in Earth-like weather. Titan has its own climate, which results in the making of dunes, rivers, lakes and seas. Although not made from water, this moon looks a lot like the place mankind calls home. 
	
	\subsection{Problem description}
	
	The objective of this project is to simulate the Solar System, in that simulation: launch a rocket to Titan, land it safely on its surface and bring it back to Earth.  All the travel has to be optimized to reduce the total cost of this exploratory mission, this means finding the optimal trajectory and reducing the overall fuel usage.
	In order to start tackling this problem, a physically correct simulation of the Solar System is needed. Using this model probes are launched from Earth. These probes behaves like cannonballs, launched from a cannon on Earth's surface. These so called exploratory missions help explore the workings of the physical simulation of the Solar System, which is based on gravity, and to develop an algorithm to calculate the trajectory of the probe.\\    
	
	Landing a rocket safely is a difficult mission. In order to land a rocket on a planet, it has to orbit the planet first. Once this is done, it has to align its direction so it can enter the orbit. In this phase of the landing the rocket is influenced by atmospheric factors, such as the wind. It needs to stabilize the rotation the wind inflicts, as the rocket can land straight and safe. It is important to make the rocket land with the optimal alignment and a minimal velocity, because obviously it is unsafe to land with a high speed.\\
	
	Afterwards, the rocket is launched from Titan towards mother Earth in order to land on it after a few years of traveling. 
	...
	To digitally simulate the Solar System, Newton's gravitational law is used. The model is based on the equation $\vec{F} = \frac{\vec{r}}{|\vec{r}|} G \dfrac{m_1 m_2}{|\vec{r}|^2} $. This means that the gravitational force F is equal to the product of the masses of both objects, divided by the distance squared, multiplied by the gravitational constant G. Or in other words: the bigger the object?s masses are and the closer they are to each other, the bigger the gravitational force between them will be.\\
	...
	The whole simulated Solar System works on just one equation: Newton's gravitational theorem. Using this formula, the acceleration of the planets is calculated. Then, from the acceleration, the velocity of the celestial bodies can be found using ordinary differential equations solvers. Furthermore and using the same solvers, the planets? position is also computable from the velocity. In order to implement the gravity and actually make the celestial bodies orbit, solving these equations is required. // 
	There are a lot of numerical approaches to solve these. The report will go in depth about the methods used in the program. 
	
	
	
	\paragraph{Euler}
	Euler's method is a numerical method that solves first order differential equations. It is one of the simplest ODE-solvers, yet still relatively accurate for a small number of iterations.
	\begin{flalign}
	&\dot{y}(t) = f(t, y)\\
	...
	It is not possible to obtain information about the acceleration of the celestial bodies from NASA, so it is necessary to calculate it. This 3 point difference method is used during the initialization of Adams-bashforth?s method, by calculating the acceleration as derivative of the velocity. The acceleration is defined as $\vec{a} = \frac{d\vec{v}}{dt} $. So the values of the acceleration can be calculated as following:
	...
	Then only Newton?s gravitational law is used to update the acceleration of the planets and the shuttle in the simulation.
	
	\subsection{Spaceship}
	\subsubsection{Shape}
	The rocket is designed to be a spherical shell that is hollow inside. This design makes the calculations about the forces on the spaceship easier. Those forces include the wind, the engines, the atmosphere and the parachute. Note that the effect of the wind is partially determined by the surface area. The choice of this shape (although not the most aerodynamic, which leads to increased costs) does help to control the rotation, increases the shuttle's stability and efficiency. The external radius is 20m, while the inner radius is 5m. Then the space inside is used by the astronauts and the fuel is uniformly distributed in the remaining space.
	...
	\footnote{Olinde Rodrigues, "Des lois géométriques qui régissent les déplacements d'un système solide dans l'espace, et de la variation des coordonnées provenant de ces déplacement considérés indépendant des causes qui peuvent les produire", J. Math. Pures Appl. 5 (1840), 380?440.}
	...
	\section{Travel}    
	\subsection{Launch}
	To launch the rocket from any planet, a special launching station that launches the spacecraft with a certain initial velocity has been assumed. It is comparable to a catapult launching a projectile. The higher the velocity during the launch, the easier it is to calculate a correct course, the lower its speed, the easier it gets dragged away by other planets' gravity. Thus it is recommended to experiment with a higher speed first.
	...
	In this phase the atmosphere is taken into account, so the spaceship is influenced by wind, atmospheric drag and a parachute used to reduce the landing velocity. Note that the atmosphere  is about 1.5 more dense than Earth?s atmosphere \footnote{Lindal, G. F.; Wood, G. E.; Hotz, H. B.; Sweetnam, D. N.; Eshleman, V. R.; Tyler, G. L. (1983-02-01). "The atmosphere of Titan: An analysis of the Voyager 1 radio occultation measurements". Icarus. 53 (2): 348?363. doi:10.1016/0019-1035(83)90155-0. ISSN 0019-1035.} and 600km thick \footnote{Retrieved from https://www.space.com/15257-titan-saturn-largest-moon-facts-discovery-sdcmp.html
	}.
	The parachute simply adds an extra acceleration which is opposite to the spaceship velocity and proportional to the atmosphere density. The atmospheric drag is proportional to the altitude, the atmosphere density and the spaceship area. Both of these effects are affecting the spaceship only when it is in the atmosphere.
	...
	This controller does not take into account changes of the environment but tries to reach the correct state at each phase of the landing.
	...
	This algorithm is designed to find the quickest route to Titan (with a specified travelling time), only using an initial velocity and the gravitational force. The algorithm is actually working on finding a trajectory for a trip lasting one year. 
	Unfortunately, there is an absence of formal proof that the trajectory computed by the algorithm is the best one. The spaceship is following the most straightforward path to get to Titan?s orbit but there could exist some other faster routes.
	
	\subsection{Maths optimizations}
	As mentioned earlier, two different equation solvers have been used in the program. The first order Euler method was first implemented. It was later on upgraded to the fourth order Adams-Bashforth method, which proved to be more accurate than the previous one. The results of this improvement are presented in section 9 table 5.\\
	...
	Since the rocket travels with a speed of approximately 84.99 km/s, some experiences with lower speeds to make it more realistic could be interesting (Huygens travelled at an average velocity of 16.4 km/s \footnote{Retrieved from https://www.esa.int/Our\_Activities/Space\_Science/Cassini-Huygens/Cassini-Huygens\_Frequently\_Asked\_Questions\_FAQs}, but with a travel of 7 years instead of only 1). To do so, the initial velocity with which the rocket is launched has been reduced. When changing the velocity to realistic speeds, the shuttle ended up orbiting around the Sun or Earth itself. This lead to an interesting weakness of the program. The actual hypothesis is that it gets stuck into the orbit of either the Sun or Earth, due to not using engines during the travel. In ?real life?, a rocket uses its own engine to leave Earth orbit and to steer itself to its desired location. Simulating this process would mean rethinking the whole launching and trajectory approaches, thus would be very time consuming. We see this as a challenge we would like to work on later.
	
	\subsection{Unrealistic wind}
	Since the wind model rightfully predicts a low wind on the surface, the intention is to test the controller for stormier weather as well. Unfortunately there are no data about Titan?s weather, so the strategy is to increase the influence of the wind. The results can be found in chapter 9. in table 2 and 4.
	...
	It is possible to observe that the angular velocity and the angle with respect to the ground are correct, so the feedback controller for the rotation is working fine. On the other hand the velocity controller is not working properly yet, but it has improved from the previous versions of the program (the first version was around 130).\\
	
	It is also possible to test how robust is the feedback controller by increasing the effect of the rotations generated by the wind. Here are the results (below).
	...
	%Future improvements
	We are planning to search for a more realistic launching velocity by using other celestial bodies to improve the trajectory (in particular Jupyter seems promising) and by organising a longer travel, for example a 7-year-long travel similar to the Cassini mission. In order to do this, we would have to rethink our approach to launch the rocket. It is impossible to launch with a realistic velocity and get the rocket to Titan without using its engines. Therefore we would need a completely different algorithm that uses the engines to get to the desired destination.
	If we would succeed in this, it would be possible to improve the orbit around Titan: at the moment the spaceship can complete a few orbits around the planet, but because of its speed and Saturn's gravitational force it cannot stay longer. By reducing the velocity it will be possible to orbit around Titan for any desired amount of time.
	...
	\begin{enumerate}        
		...
		\item Lindal, G. F.; Wood, G. E.; Hotz, H. B.; Sweetnam, D. N.; Eshleman, V. R.; Tyler, G. L. (1983-02-01). "The atmosphere of Titan: An analysis of the Voyager 1 radio occultation measurements". Icarus. 53 (2): 348?363. doi:10.1016/0019-1035(83)90155-0. ISSN 0019-1035.
		\item Retrieved from https://www.space.com/15257-titan-saturn-largest-moon-facts-discovery-sdcmp.html
		...
		\item https://solarsystem.nasa.gov/missions/cassini/overview/
	\end{enumerate}
	...
	\begin{figure}[h]
	\centering
	\includegraphics[width=1\linewidth]{screen_1.jpeg}
	\caption[Short]{screen-shot of the program.}
	\label{fig:screen2}
\end{figure}
	\begin{figure}[h]
	\centering
	\includegraphics[width=1\linewidth]{screen_2.jpeg}
	\caption[Short]{screen-shot of the landing.}
	\label{fig:screen1}
\end{figure}	
\end{document}
